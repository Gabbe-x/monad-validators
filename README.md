...
- every registered validator (ids are walked from 1 until the id space ends), with status derived from the three on-chain sets: `active` (consensus + snapshot), `joining` (in snapshot only, enters next epoch), `leaving` (in consensus only), `candidate` (in the execution set but outside the top 200), `inactive` (registered but in no set: total stake below 10M MON or self-delegation below the 100k MON minimum);
